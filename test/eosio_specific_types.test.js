...
test.describe('EOSIO Specific Types', () => {
    ...
    CoreAbi.debug = true;
    coreAbi.cleanup();
    coreAbi.loadAbi(contract, eosioTypesAbi);

    test('simple stringToName conversion', () => {
        const nameValue = coreAbi.stringToName("eosio.token");
    ...
    test('stringToName conversion with invalid name', () => {
        assertThrows(
            ...
            () => coreAbi.stringToName(null),
            'Should throw for invalid name format'
        );
    });

    test('name type serialization/deserialization', () => {
        globalTestRoundTrip(coreAbi, contract, 'name_type', { value: "eosio" });
        globalTestRoundTrip(coreAbi, contract, 'name_type', { value: "eosio.token" });
        globalTestRoundTrip(coreAbi, contract, 'name_type', { value: "eosio.null" });
        globalTestRoundTrip(coreAbi, contract, 'name_type', { value: "" });
        globalTestRoundTrip(coreAbi, contract, 'name_type', { value: "a" });
        globalTestRoundTrip(coreAbi, contract, 'name_type', { value: "zzzzzzzzzzzzj" });
    });

    test('asset type serialization/deserialization', () => {
        globalTestRoundTrip(coreAbi, contract, 'asset_type', { value: "1.0000 EOS" });
        globalTestRoundTrip(coreAbi, contract, 'asset_type', { value: "0.0000 EOS" });
        globalTestRoundTrip(coreAbi, contract, 'asset_type', { value: "-1.0000 EOS" });
        globalTestRoundTrip(coreAbi, contract, 'asset_type', { value: "1000000000.0000 EOS" });
    });
    ...
    test('time types serialization/deserialization', () => {
        ...
        globalTestRoundTrip(coreAbi, contract, 'time_types', {
        ...
        assertThrows(
            ...
            () => coreAbi.jsonToHex(contract, 'time_types', {
    ...
    test('public key serialization/deserialization', () => {
        ...
        const hex = coreAbi.jsonToHex(contract, 'public_key_type', data);
        const result = coreAbi.hexToJson(contract, 'public_key_type', hex);
        ...
        assertThrows(
            ...
            () => coreAbi.jsonToHex(contract, 'public_key_type', { value: "invalid key" }),
            'Should reject invalid public key format'
        );

        assertThrows(
            /Failed to convert JSON to hex.*Expected key/i,
            () => coreAbi.jsonToHex(contract, 'public_key_type', { value: "EOS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CZ" }),
