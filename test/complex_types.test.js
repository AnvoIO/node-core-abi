...
test.describe('Variant Types and Complex Structures', () => {
    ...
    CoreAbi.debug = true;
    coreAbi.cleanup();
    coreAbi.loadAbi(contract, complexAbi);

    test('basic struct serialization/deserialization', () => {
        globalTestRoundTrip(coreAbi, contract, 's1', { x1: 10 });
        globalTestRoundTrip(coreAbi, contract, 's2', { y1: -5, y2: 5 });

        assertThrows(
            /number is out of range/i,
            () => coreAbi.jsonToHex(contract, 's1', { x1: 128 }),
            'Should reject int8 out of range'
        );

        assertThrows(
            /Expected field/i,
            () => coreAbi.jsonToHex(contract, 's2', { y1: 0 }),
            'Should reject struct with missing required field'
        );
    });

    test('struct with optionals serialization/deserialization', () => {
        globalTestRoundTrip(coreAbi, contract, 's_optional', { a1: 10, b1: [1, 2, 3] });

        const optionalData = { a1: null, b1: [1, 2, 3] };
        const hexWithNull = coreAbi.jsonToHex(contract, 's_optional', optionalData);
        const jsonWithNull = coreAbi.hexToJson(contract, 's_optional', hexWithNull);
        ...
        assertThrows(
            /Expected field/i,
            () => coreAbi.jsonToHex(contract, 's_optional', { b1: [] }),
            'Should throw if optional field `a1` is missing but `b1` is present'
        );
        globalTestRoundTrip(coreAbi, contract, 's_optional', { a1: null, b1: [] });
    });

    test('variant type serialization/deserialization', () => {
        globalTestRoundTrip(coreAbi, contract, 'my_variant', ["int8", 10]);
        globalTestRoundTrip(coreAbi, contract, 'my_variant', ["s1", { x1: 20 }]);
        globalTestRoundTrip(coreAbi, contract, 'my_variant', ["s2", { y1: 30, y2: 40 }]);

        assertThrows(
            /Invalid type for variant/i,
            () => coreAbi.jsonToHex(contract, 'my_variant', ["unknown_type", 10]),
            'Should reject variant with unknown type'
        );

        assertThrows(
            /Expected field/i,
            () => coreAbi.jsonToHex(contract, 'my_variant', ["s1", { invalid: 10 }]),
            'Should reject variant with invalid struct field'
        );

        assertThrows(
            /Expected three string arguments/i,
            () => coreAbi.jsonToHex(contract, 'my_variant', 10),
            'Should reject variant that is not an array'
        );
    });

    test('struct with variant serialization/deserialization', () => {
        globalTestRoundTrip(coreAbi, contract, 's_with_variant', { v1: ["int8", 10], z1: 20 });
        globalTestRoundTrip(coreAbi, contract, 's_with_variant', { v1: ["s1", { x1: 30 }], z1: 40 });
        globalTestRoundTrip(coreAbi, contract, 's_with_variant', { v1: ["s2", { y1: 50, y2: 60 }], z1: 70 });

        assertThrows(
            /Expected field/i,
            () => coreAbi.jsonToHex(contract, 's_with_variant', { z1: 20 }),
            'Should reject struct missing variant field'
        );
    });

    test('nested complex structure serialization/deserialization', () => {
        globalTestRoundTrip(coreAbi, contract, 's_nested', {
            z1: 10,
            z2: ["int8", 20],
            z3: { a1: 30, b1: [40, 50] }
        });
        ...
        assertThrows(
            /Expected field/i,
            () => coreAbi.jsonToHex(contract, 's_nested', { z1: 10, z2: ["int8", 20] }),
