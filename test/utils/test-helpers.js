...
export function assertThrows(expectedError, fn, message) {
    try {
        ...
        if (!expectedError && e.message.includes(CoreAbi.logTag)) {
 ...
 * Assumes coreAbi.jsonToHex handles JSON.stringify internally.
 * @param {CoreAbi} coreAbi - CoreAbi instance.
 * @param {string} contract - Contract name.
 * @param {string} type - Type name.
 * @param {Object} data - Data to convert (JavaScript object).
 * @returns {string} Hex representation of data.
 */
export function testRoundTrip(coreAbi, contract, type, data) {
    const hex = coreAbi.jsonToHex(contract, type, data);
    assert.ok(typeof hex === 'string' && hex.length > 0, `Serialization should produce hex for ${type}`);
    const roundTripped = coreAbi.hexToJson(contract, type, hex);
